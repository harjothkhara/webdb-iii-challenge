...
router.get("/:id", async (req, res) => {
    ...
    try {
        ...
        if (Student) {
            ...
            res.status(404).json({ message: "Student with specified ID does not exist" });
        }
    } catch(error) {
        res.status(500).json({ message: `Student request failed ${error}` });
    }
});
...
router.post("/", async (req, res) => {
    ...
    if (!Student.name) {
        ...
        try{
            ...
            res.status(500).json({ message: `Your student could not be posted ${error}.` });
        }
    }
});
